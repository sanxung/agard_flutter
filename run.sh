...
run_indexes=(2)  # indexes of mach numbers to submit for simulation
...
main_path=$(pwd)
study_name="results_every-2-steps"
...
if [ "$run" = true ] && [ "$run_type" == "2" ]; then  # map mode shapes
    ...
    ln -s $main_path/$study_name/mach-${machs[$run_ind]}/speed_index-${speed_ind}/steady/*massoud_body1.dat .
